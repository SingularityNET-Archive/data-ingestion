-- Materialized views for KPI aggregates used by the dashboard
-- These should be created after the read-only views in views.sql
...
SELECT
  ...
  (SELECT COUNT(DISTINCT source_id) FROM public.meeting_summary_view) AS sources_count,
